...
from scene import Scene, run, ShapeNode, LabelNode, Rect, Node, SpriteNode, Texture, Point
...
from view3d import IsometricCloud
...
class EliteScene(Scene):
    ...
    def setup(self):
        ...
        temp_state = {
         'myship': SimpleNamespace(**{'max_speed': 255,
                                      'max_fuel': 255,
                                      'max_rol': 255,
                                      'max_climb': 255})}
                            
        temp_state = SimpleNamespace(**temp_state)
        self.input_queue = queue.Queue()
        self.active_touches = {}
        self.keypad = EliteKeypad(frame=(cs.KEYBOARD_X, cs.KEYBOARD_Y,
                                  cs.KEYBOARD_W, cs.KEYBOARD_H),
        ...
        self.fire_button = SpriteNode(Texture(image),
                                      ...
                                      parent=self)
        self.msg = LabelNode('', position=(0.8 * W, 0.65 * H), color=cs.WHITE, parent=self)
        ...
        for keyname in ['Jump', 'Escape', 'Cancel Escape', 'ECM', 'Bomb',
                        ...
                        'Look Stbd', 'Look Fwd', 'Look Aft', 'Docking',
    ...
    def create_background(self):
        ...
        path.move_to(0, cs.GAME_H - cs.HUD_H-15)
        path.line_to(cs.FLIGHT_W + 4, cs.GAME_H - cs.HUD_H - 15)
    ...
    def draw(self):
        ...
        self.renderer.draw(objects, self.camera, self.flight_rect)
        # self.gfx.draw()
        self.cloud.objects = objects
    ...
    def touch_ended(self, touch):
        ...
        if control == 'joystick':
            ...
            self.is_joystick_active = False
            self.input_queue.put(f'>0.00,0.00')
        elif control == 'thrust':
...
class Keyboard:
    ...
    # TODO implement queue to capture mulitouches
    # could supply poll  with mode to remove from queue
    ...
    def poll(self, mode=None):
        ...
        screen_keys = {'Launch', 'Equip', 'Galaxy Chart', 'Local Chart', 'Data', 'Trade',
                       'Market',  'Status', 'Pause', 'Resume', 'Help', 'Quit'}
