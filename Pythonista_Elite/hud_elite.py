...
# LOC: 538
from scene import SpriteNode, Rect,  Node, Point, Size, ShapeNode, Scene, run
import constants as cs
import math
import ui
# from constants import logger
from game_engine import sounder
...
class Scanner(Node):
   ...
   def __init__(self, game_state, width=400, height=300):
       super().__init__()
       self.gs = game_state
       try:
...
class Compass(Node):
    ...
    # lock to station
    def __init__(self, game_state, width=70):
...
class EliteScannerView(Scene):
    ...
    def setup(self):
        ...
        self.radar_node = Scanner(None, width=cs.SCANNER_RECT.w, height=cs.SCANNER_RECT.h)
        ...
        self.compass_node = Compass(None, width=cs.COMPASS_W)
...
class HudPanel(Node):    
    ...
    def __init__(self, game_state):
        ...
        self.name = 'ecm'        
        self.color = cs.BLUE
        self.scanner = Scanner(self.gs, *cs.SCANNER_RECT.size)
        self.scanner.position = cs.SCANNER_RECT.center()
        self.add_child(self.scanner)
        self.compass = Compass(self.gs, width=cs.COMPASS_RECT.w)
