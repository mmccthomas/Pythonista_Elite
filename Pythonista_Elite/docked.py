...
class Docked:
    ...
    def __init__(self, game_state):
        ...
        self.cmdr = game_state.cmdr          # commander save-block
    ...
    def draw_fuel_limit_circle(self, c : Point2):
        ...
        gfx = self.gfx
        radius = self.cmdr.fuel / 3 * self.zoom_scale * self.SCALE
        cross_size = 1 * self.zoom_scale * self.SCALE
        cx, cy = c.as_tuple()
        gfx.draw_circle(cx, cy, radius, cs.GREEN)
    ...
    def _plot_planets_and_names(self):
        ...
        sorted_planets = gs.planet.get_closest_planets(gs.docked_planet, max_distance=self.cmdr.fuel)
    ...
    def display_galactic_chart(self):
        ...
        gfx.clear_display()
        gfx.display_centre_text(0, f"GALACTIC CHART {self.cmdr.galaxy_number + 1}", 140, cs.GOLD)        
        self.draw_fuel_limit_circle(self._to_screen(Point2(gs.docked_planet.x, gs.docked_planet.y)))
    ...
    def display_commander_status(self):
        ...
        if not gs.witchspace:    # selfcmdr.ship_x, self.cmdr.ship_y)
    ...
    def display_stock_price(self, i):
        gfx = self.gfx
        cmdr = self.cmdr
        sm = [SimpleNamespace(**item) for item in self.gs.trade.stock_market]
        self.sm = sm
    ...
    def buy_stock(self):
        ...
        if not self.gs.docked:
            return
        item = self.sm[self.hilite_market]
    ...
    def display_market_prices(self):
        ...
        gs.msg.text = f'{self.hilite_market=}'
        gfx.text_render()
    ...
    def list_equip_prices(self):
        ...
        self.equip_stock[0].price = (70 - self.cmdr.fuel) * 2

        y = self.EQUIP_START_ROW
        item_list = []
        for item in self.equip_stock:
            item.canbuy = int(
                not self.equip_present(item.type) and item.price <= self.cmdr.credits
            )
