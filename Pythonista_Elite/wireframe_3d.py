...
class Vector3:
    ...
    def normalize(self):
        return self / self.length() if self.length() else Vector3()
...
class Renderer:
    ...
    def draw(self, objects, camera, screen_size):
        ...
        for obj in visible:
            ...
            screen_pts = [self._project(v, fl, sw, sh, camera) for v in cam_verts]
    ...
    def explode(self, obj, camera, screen_size):
        ...
        if True:  # obj.visible:
            ...
            for ei, (i1, i2) in enumerate(obj.edges):
                ...
                p1 = self._project(self._to_camera(v1, camera), fl, sw, sh, camera)
                p2 = self._project(self._to_camera(v2, camera), fl, sw, sh, camera)
                ...
                if p1 and p2:
                    ...
                    scene.stroke(*edge_color)
                    scene.rect(0, 0, 0, 0)
                    scene.line(p1[0], p1[1], p2[0], p2[1])
...
class SpaceFlight(scene.Scene):
    ...
    def draw(self):
        ...
        all_objs = [ms.obj for ms in self.moving_ships]
