...
class ButtonRepeater:
    def __init__(self, button, action, interval=0.1, delay=0.5):
        self.button = button
        self.action = action
        ...
        # Override the button's touch behavior
        # Note: We use the button's internal 'action' slot for the repeat logic
        self.button.action = self.button_tapped_standard
        
        # To detect "down" and "up", we need to use the view's touch methods
        # We redirect the button's touch handlers to our logic
        self.button.touch_began = self.touch_began
        self.button.touch_ended = self.touch_ended

    def touch_began(self, touch):
        self.is_pressing = True
        self.action(self.button)  # Initial fire
        ui.delay(self.start_repeating, self.delay)
    ...
    def start_repeating(self):
        if self.is_pressing:
            ...
            ui.delay(self.start_repeating, self.interval)
            
    def button_tapped_standard(self, sender):
        # We disable the standard 'action' to prevent double-firing
        pass
    ...
    def __init__(self, frame=(0, 0, 300, 300), action=None,
        ...
        self.layout_data = [
            [('Launch', 1, 'cyan'), ('Trade', 1, 'cyan'), ('Equip', 1, 'cyan'),
             ...
             ('Data', 1, 'cyan')],
            [(' ', 1, '#fb9bff'), ('Pause', 1, '#fb9bff'), ('Quit', 1, '#fb9bff'),
             ('Find', 1, 'cyan'), (' ', 1, 'cyan'), ('OK', 1, '#ff8080'), ('Cancel', 1, '#ff8080')],
            ...
            [('Docking', 1, 'lightgreen'), ('New Galaxy', 1, 'lightgreen'), (' ', 1, '#fb9bff'),
             (' ', 1, '#fb9bff'), (' ', 1, '#fb9bff'), ('Up', 1, 'yellow'),
             (' ', 1, 'lightgreen')],
            [('Look Port', 1, 'cyan'), ('Look Stbd', 1, 'cyan'), ('Look Fwd', 1, 'cyan'),
             ('Look Aft', 1, 'cyan'), ('Left', 1, 'yellow'), ('Select', 1, '#ff8080'),
             ('Right', 1, 'yellow')],
            [(' ', 1, 'orange'), (' ', 1, 'orange'), ('Fire Laser', 1, 'orange'),
             (' ', 1, 'orange'), (' ', 1, 'cyan'), ('Down', 1, 'yellow'), (' ', 1, 'cyan')]
        ]
        REPEATING_KEYS = {}  # 'Up', 'Down', 'Left', 'Right', 'Fire laser'}
        self.btns = {}
        for row in self.layout_data:
            for char, weight, color in row:
                ...
                btn.weight = weight
                if char in REPEATING_KEYS:
                    ButtonRepeater(btn, self.action or self.button_tapped)
                else:
