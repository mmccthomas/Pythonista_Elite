...
class Starfield:
    ...
    def front_starfield(self):
        ...
        delta = 50.0 if self.warp_stars else float(self.gs.flight_speed)
        alpha = float(self.gs.flight_roll) / 256.0
        beta = float(self.gs.flight_climb)
        delta /= 2.0
        cx = self.center_x
        cy = self.center_y
    ...
    def side_starfield(self):
        ...
        nstars = 3 if self.gs.witchspace else self.n_stars
        delta = 50.0 if self.warp_stars else float(self.gs.flight_speed)
        alpha = float(self.gs.flight_roll)
        beta = float(self.gs.flight_climb)
        cx = self.center_x
        cy = self.center_y
