from scene import Point, stroke, stroke_weight, fill, line, no_stroke,ellipse, rect
import math
...
class IsometricCloud():
    ...
    def draw(self):
        ...
        for obj in self.objects:
            self.dot_size = size = math.sqrt(obj.model.header['Targetable area'])/ 10
            fill(obj.model.color)
            no_stroke()
            rect(0, 0, 0, 0)
            screen_pt = self.project(*obj.location.to_tuple())
